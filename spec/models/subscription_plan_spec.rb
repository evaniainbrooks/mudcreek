...
RSpec.describe SubscriptionPlan, type: :model do
  ...
  describe "validations" do
    it "requires a name" do
      plan = SubscriptionPlan.new(amount_cents: 1000, subscription_type: :month_to_month)
    ...
    it "enforces name uniqueness within a tenant" do
      ...
      plan = SubscriptionPlan.new(name: "Basic", amount_cents: 1000, subscription_type: :month_to_month)
      expect(plan).not_to be_valid
      expect(plan.errors[:name]).to be_present
    end

    it "requires amount_cents greater than zero" do
      plan = SubscriptionPlan.new(name: "Bad", amount_cents: 0, subscription_type: :month_to_month)
      expect(plan).not_to be_valid
