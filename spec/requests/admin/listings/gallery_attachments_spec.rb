...
RSpec.describe "Admin::Listings::GalleryAttachments", type: :request do
  before do
    host! "example.com"
  ...
  let(:gallery) { create(:gallery, listing: listing) }
  let!(:attachment) do
  ...
  describe "DELETE /admin/listings/:listing_hashid/gallery_attachments/:id" do
    it "redirects to the listing edit page" do
      delete admin_listing_gallery_attachment_path(listing_hashid: listing.hashid, id: attachment.id)
    ...
    context "with a non-existent attachment id" do
      it "returns 404" do
        delete admin_listing_gallery_attachment_path(listing_hashid: listing.hashid, id: 0)
