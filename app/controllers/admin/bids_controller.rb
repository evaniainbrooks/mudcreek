class Admin::BidsController < Admin::BaseController
  ...
  def update
    ...
    return redirect_to admin_listing_path(@bid.auction_listing.listing), alert: t(".alert") unless state

    @bid.update!(state: state)
    ProxyBiddingService.resolve(@bid.auction_listing)
    redirect_to admin_listing_path(@bid.auction_listing.listing), notice: t(".notice", state: state)
